...
final class WP_Sms_Admin {
	...
	/**
	 * Main WP_Sms_Admin Instance
	 *
	 * Ensures only one instance of WP_Sms_Admin is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main WP_Sms_Admin instance
	 */
	...
	public function settings_screen () {
		global $title;
		$sections = WP_Sms()->settings->get_settings_sections();
		$tab = $this->_get_current_tab( $sections );
		?>
		<div class="wrap wp-sms-wrap">
			<?php
				echo $this->get_admin_header_html( $sections, $title );
			?>
			<form action="options.php" method="post">
				<?php
					settings_fields( 'wp-sms-settings-' . $tab );
					do_settings_sections( 'wp-sms-' . $tab );
					submit_button( __( 'Save Changes', 'wp-sms' ) );
				?>
			</form>
		</div><!--/.wrap-->
		<?php
	...
	public function register_settings () {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				register_setting( 'wp-sms-settings-' . sanitize_title_with_dashes( $k ), 'wp-sms-' . $k, array( $this, 'validate_settings' ) );
				add_settings_section( sanitize_title_with_dashes( $k ), $v, array( $this, 'render_settings' ), 'wp-sms-' . $k, $k, $k );
			}
		}
	...
	public function render_settings ( $args ) {
		$token = $args['id'];
		$fields = WP_Sms()->settings->get_settings_fields( $token );
		...
		if ( 0 < count( $fields ) ) {
			foreach ( $fields as $k => $v ) {
				...
				add_settings_field( $k, $v['name'], array( WP_Sms()->settings, 'render_field' ), 'wp-sms-' . $token , $v['section'], $args );
			}
		}
	} // End render_settings()
	...
	public function validate_settings ( $input ) {
		$sections = WP_Sms()->settings->get_settings_sections();
		$tab = $this->_get_current_tab( $sections );
		return WP_Sms()->settings->validate_settings( $input, $tab );
	...
	public function get_admin_header_html ( $sections, $title ) {
		$defaults = array(
							'tag' => 'h2',
							'atts' => array( 'class' => 'wp-sms-wrapper' ),
							'content' => $title
						);
	...
	private function _get_admin_header_data ( $sections, $title ) {
		$response = array( 'tag' => 'h2', 'atts' => array( 'class' => 'wp-sms-wrapper' ), 'content' => $title );
		...
		if ( is_array( $sections ) && 1 < count( $sections ) ) {
			...
			foreach ( $sections as $key => $value ) {
				...
				$response['content'] .= '<a href="' . admin_url( 'options-general.php?page=wp-sms&tab=' . sanitize_title_with_dashes( $key ) ) . '" class="' . esc_attr( $class ) . '">' . esc_html( $value ) . '</a>';
			}
		}

		return (array)apply_filters( 'wp-sms-get-admin-header-data', $response );
