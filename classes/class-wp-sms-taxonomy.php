...
class WP_SMS_Taxonomy {
	...
	public function __construct ( $post_type = 'thing', $token = 'thing-category', $singular = '', $plural = '', $args = array() ) {
		...
		if ( '' == $this->singular ) $this->singular = __( 'Category', 'wp-sms' );
		if ( '' == $this->plural ) $this->plural = __( 'Categories', 'wp-sms' );
	...
	private function _get_default_labels () {
		return array(
			    'name'                => sprintf( _x( '%s', 'taxonomy general name', 'wp-sms' ), $this->plural ),
			    'singular_name'       => sprintf( _x( '%s', 'taxonomy singular name', 'wp-sms' ), $this->singular ),
			    'search_items'        => sprintf( __( 'Search %s', 'wp-sms' ), $this->plural ),
			    'all_items'           => sprintf( __( 'All %s', 'wp-sms' ), $this->plural ),
			    'parent_item'         => sprintf( __( 'Parent %s', 'wp-sms' ), $this->singular ),
			    'parent_item_colon'   => sprintf( __( 'Parent %s:', 'wp-sms' ), $this->singular ),
			    'edit_item'           => sprintf( __( 'Edit %s', 'wp-sms' ), $this->singular ),
			    'update_item'         => sprintf( __( 'Update %s', 'wp-sms' ), $this->singular ),
			    'add_new_item'        => sprintf( __( 'Add New %s', 'wp-sms' ), $this->singular ),
			    'new_item_name'       => sprintf( __( 'New %s Name', 'wp-sms' ), $this->singular ),
			    'menu_name'           => sprintf( __( '%s', 'wp-sms' ), $this->plural )
			  );
