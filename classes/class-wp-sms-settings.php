 ...
 * @author Jeffikus
 */
final class WP_SMS_Settings {
	/**
	 * WP_SMS_Admin The single instance of WP_SMS_Admin.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	/**
	 * Main WP_SMS_Settings Instance
	 *
	 * Ensures only one instance of WP_SMS_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main WP_SMS_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true === (bool)apply_filters( 'wp-sms-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'wp-sms-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'wp-sms-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= WP_SMS()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'wp-sms-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'wp-sms-new-line-description-fields', array( 'textarea', 'select' ) ) ) ) {
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'wp-sms' );
		$settings_sections['special-fields'] = __( 'Special Fields', 'wp-sms' );
		...
		return (array)apply_filters( 'wp-sms-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
				...
				$settings_fields['text'] = array(
												'name' => __( 'Example Text Input', 'wp-sms' ),
												'type' => 'text',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'wp-sms' )
											);
				$settings_fields['textarea'] = array(
												'name' => __( 'Example Textarea', 'wp-sms' ),
												'type' => 'textarea',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'wp-sms' )
				...
				$settings_fields['checkbox'] = array(
												'name' => __( 'Example Checkbox', 'wp-sms' ),
												'type' => 'checkbox',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'wp-sms' )
				...
				$settings_fields['radio'] = array(
												'name' => __( 'Example Radio Buttons', 'wp-sms' ),
												'type' => 'radio',
												'default' => '',
												'section' => 'standard-fields',
												'options' => array(
																	'one' => __( 'One', 'wp-sms' ),
																	'two' => __( 'Two', 'wp-sms' ),
																	'three' => __( 'Three', 'wp-sms' )
															),
												'description' => __( 'Place the field description text here.', 'wp-sms' )
											);
				$settings_fields['select'] = array(
													'name' => __( 'Example Select', 'wp-sms' ),
													'type' => 'select',
													'default' => '',
													'section' => 'standard-fields',
													'options' => array(
			...
			case 'special-fields':
				...
				$settings_fields['select_taxonomy'] = array(
													'name' => __( 'Example Taxonomy Selector', 'wp-sms' ),
		...
		return (array)apply_filters( 'wp-sms-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'wp-sms-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'wp-sms-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'wp-sms-' . $v, array() );
